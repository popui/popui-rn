  ...
  showDefault(transparent, modal, text) {
    let overlayView = (
      <Overlay.View
        ...
        >
        <View style={{backgroundColor: transparent ? '#333' : defaultColor, padding: 40, borderRadius: 15, alignItems: 'center'}}>
          <CellLabel type='danger' size='xl' text={text} />
          {modal ? <View style={{height: 20}} /> : null}
          {modal ? <WeuiButton title='Close' onPress={() => this.overlayView && this.overlayView.close()} /> : null}
        </View>
      </Overlay.View>
    );
  ...
  showPop(type, modal, text) {
    let overlayView = (
      <Overlay.PopView
        ...
        >
        <View style={{backgroundColor: defaultColor, minWidth: 260, minHeight: 180, borderRadius: 15, justifyContent: 'center', alignItems: 'center'}}>
          <CellLabel type='title' size='xl' text={text} />
          {modal ? <View style={{height: 60}} /> : null}
          {modal ? <WeuiButton title='Close' onPress={() => this.overlayPopView && this.overlayPopView.close()} /> : null}
        </View>
      </Overlay.PopView>
    );
    Overlay.show(overlayView);
  ...
  showPull(side, modal, text, rootTransform) {
    let overlayView = (
      <Overlay.PullView side={side} modal={modal} rootTransform={rootTransform} ref={v => this.overlayPullView = v}>
        <View style={{backgroundColor: defaultColor, minWidth: 300, minHeight: 260, justifyContent: 'center', alignItems: 'center'}}>
          <CellLabel type='title' size='xl' text={text} />
          {modal ? <View style={{height: 60}} /> : null}
          {modal ? <WeuiButton title='Close' onPress={() => this.overlayPullView && this.overlayPullView.close()} /> : null}
        </View>
      </Overlay.PullView>
    );
    Overlay.show(overlayView);
  ...
  showMulti() {
    let overlayView = (
      <Overlay.PullView modal={false}>
        <View style={{backgroundColor: defaultColor, minWidth: 200, minHeight: 260, justifyContent: 'center', alignItems: 'center'}}>
          <CellLabel type='title' size='xl' text='Overlay' />
          <View style={{height: 60}} />
          <WeuiButton title='New overlay' onPress={() => this.showDefault(false, true, 'New overlay')} />
        </View>
      </Overlay.PullView>
    );
    Overlay.show(overlayView);
