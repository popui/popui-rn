...
class Swiper extends Component {
  ...
  shouldComponentUpdate = (nextProps, nextState) => {
    ...
    const cardsLenChanged = props.cards.length != nextProps.cards.length
    if (cardsLenChanged) {
      return true
    }
    const propsChanged =
  ...
  rebuildStackAnimatedValues = (cards, cardIndex) => {
    ...
    cards.slice(cardIndex).forEach((card, index) => {
      ...
      stackPositionsAndScales[`stackPosition${dIndex}`] = new Animated.Value(
        this.props.stackSeparation * factor
      )
      stackPositionsAndScales[`stackScale${dIndex}`] = new Animated.Value(
  ...
  calculateCardIndexes = (firstCardIndex, cards) => {
    debug('calculateCardIndexes', { firstCardIndex, state: this.state })
    ...
    const rtn = { firstCardIndex, secondCardIndex, previousCardIndex }
    debug('calculateCardIndexes', rtn)
    return rtn
  }
  ...
  animateStack = () => {
    ...
    for (var index = secondCardIndex; index <= lastCardIndex; index++) {
      if (
        this.state[`stackPosition${index}`] &&
        this.state[`stackScale${index}`]
      ) {
  ...
  jumpToCardIndex = (newCardIndex: number) => {
  ...
  setCardIndex = (newCardIndex: number, swipedAllCards: boolean) => {
    this.setState(
      {
  ...
  calculateOverlayLabelStyle = () => {
    let overlayLabelStyle = this.props.overlayLabels[this.state.labelType].style
      .label
  ...
  calculateOverlayLabelWrapperStyle = () => {
    let dynamicStyles = this.props.overlayLabels[this.state.labelType].style
      .wrapper
  ...
  calculateStackCardZoomStyle = (index: number) => [
    styles.card,
    this.cardStyle,
    {
      zIndex: index * -1,
  ...
  renderChildren = () => {
    ...
    return (
      <View
        ...
        style={[styles.childrenViewStyle, { zIndex }]}
      >
        {children}
      </View>
    )
  ...
  pushCardToStack = (renderedCards: any[], index: number, key: string) => {
    ...
    renderedCards.push(
      ...
      >
        {firstCard && this.renderOverlayLabel()}
        {stackCard}
      </Animated.View>
    )
  }
